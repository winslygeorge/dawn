...
function TokenCleaner:new(name, interval, qoleng_server)
...
function TokenCleaner:start()
    if self.timer_id then
        self.qoleng_server:clearTimer(self.timer_id)
    end

    self.timer_id = self.qoleng_server:setInterval(function()
        -- log debug starting token cleanup_all
        self.qoleng_server.logger:log(log_level.DEBUG, "[TokenCleaner] Starting token cleanup.", "TokenCleaner", self.name)
        local count = require("auth.token_store").cleanup_all()
        if count > 0 then
            self.qoleng_server.logger:log(log_level.INFO, "[TokenCleaner] Removed " .. count .. " expired token(s).", "TokenCleaner", self.name)
        end
    end, self.interval)
